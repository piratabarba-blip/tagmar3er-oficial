...
const snapshot = app._preview;
app._formState = {treasureMode:'magical',moneyMoBonus:20};
app._objectSelections = {ring:'Anel'};
app._objectPoolSelections = new Set(['ring|||Anel']);
app._sectionState = {money:true};
let rendered = 0, saved = 0;
app.render = () => rendered++;
game.settings.set = async (_id,key,value) => {
  assert.equal(key,'tesourosObjetosSelecionados');
  assert.deepEqual(value,{items:[]});
  saved++;
};
globalThis.Dialog = {confirm:async config => {assert.equal(config.defaultYes,false);return false;}};
assert.equal(await app._resetDefaults(),false);
assert.equal(app._preview,snapshot);
assert.equal(saved,0);
assert.equal(rendered,0);
Dialog.confirm = async () => true;
assert.equal(await app._resetDefaults(),true);
assert.equal(saved,1);
assert.equal(rendered,1);
assert.equal(app._formState,null);
assert.equal(app._lastRoll,null);
assert.deepEqual(app._preview,[]);
assert.deepEqual(app._sectionState,{});
assert.deepEqual(app._objectSelections,{});
assert.equal(app._objectPoolSelections.size,0);
console.log('PASS: restore defaults requires confirmation, clears generation state, and only persists object selection.');
